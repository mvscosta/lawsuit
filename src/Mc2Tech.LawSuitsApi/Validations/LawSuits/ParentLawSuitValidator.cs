...
namespace Mc2Tech.LawSuitsApi.Validations.LawSuits
{
    ...
    /// </summary>
    public class ParentLawSuitValidator : PropertyValidator
    {
        ...
        public ParentLawSuitValidator(ApiDbContext apiDbContext)
        {
        ...
        protected override ValidationFailure CreateValidationError(PropertyValidatorContext context)
        {
            ...
            var failure = new ValidationFailure(
                ...
                $"{context.PropertyName} '{value}' invalid parent law suit"               
            );

            failure.ErrorCode = "ParentLawSuitValidator";
        ...
        protected async override Task<bool> IsValidAsync(PropertyValidatorContext context, CancellationToken ct)
        {
            ...
            if (parent.ParentLawSuit.ParentLawSuit.ParentLawSuit.ParentLawSuitId != null)
            {
                return false;
            }

            // only one reference in the hierarchy
            if (parent.Id == parent.ParentLawSuit.ParentLawSuitId || parent.Id == parent.ParentLawSuit.ParentLawSuit.ParentLawSuitId)
            {
                return false;
            }
        ...
        //protected async Task<int> DepthHierarchy(Guid parentId, Guid id, int count)
        //{
        //    if (count > 4)
        //        return 4;
